...
// ═══════════════════════════════════════════════════════════════
// /book — Discovery Call Booking Intake
// ═══════════════════════════════════════════════════════════════
...
/// POST /book/submit — store booking, email operator, redirect to /book/confirmed
pub async fn book_submit(
    State(s): State<Arc<t0>>,
    ConnectInfo(addr): ConnectInfo<SocketAddr>,
    headers: HeaderMap,
    Form(f): Form<BookForm>,
) -> impl IntoResponse {
    // Honeypot
    if f.website.as_deref().unwrap_or("").len() > 0 {
        return Redirect::temporary("/book/confirmed?ref=bot").into_response();
    }

    // Minimal validation — required fields non-empty, email has an @, date matches YYYY-MM-DD.
    if f.name.trim().is_empty()
        || f.email.trim().is_empty()
        || !f.email.contains('@')
        || f.date.trim().is_empty()
        || f.time.trim().is_empty()
        || f.topic.trim().is_empty()
    {
        return Redirect::temporary("/book?error=missing").into_response();
    }
    ...
    // Store in SQLite first so we never lose the booking even if email fails.
    if let Some(pool) = &s.intake_pool {
        let _ = sqlx::query(
            "INSERT INTO bookings (id, name, email, phone, date, time, topic, submitted_at, ip_address, user_agent, notify_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        )
        .bind(&id)
        .bind(f.name.trim())
        .bind(f.email.trim())
    ...
    // Fire email notification off the request thread. Failure is logged, not surfaced.
    let booking = super::mailer::BookingEmail {
        id: id.clone(),
        name: f.name.clone(),
        email: f.email.clone(),
        phone: f.phone.clone(),
        date_iso: f.date.clone(),
        time_label: f.time.clone(),
        topic: f.topic.clone(),
        ip: ip.clone(),
    };
    let pool_opt = s.intake_pool.clone();
    let notify_id = id.clone();
    tokio::spawn(async move {
        let send_result =
            tokio::task::spawn_blocking(move || super::mailer::send_booking(&booking))
                .await
                .unwrap_or_else(|e| Err(format!("spawn_blocking failed: {}", e)));
        let status = match &send_result {
            Ok(()) => {
                tracing::info!("booking {} email notification sent", notify_id);
                "sent".to_string()
            }
            Err(e) => {
                tracing::warn!("booking {} email notification failed: {}", notify_id, e);
                format!("failed: {}", e)
            }
        };
        if let Some(pool) = &pool_opt {
            let _ = sqlx::query("UPDATE bookings SET notify_status = ? WHERE id = ?")
                .bind(&status)
                .bind(&notify_id)
                .execute(pool)
                .await;
...
/// GET /book/confirmed — confirmation page after a booking submit
pub async fn book_confirmed(
    Query(q): Query<std::collections::HashMap<String, String>>,
) -> Html<String> {
    let ref_id = q.get("ref").map(|s| html_escape(s)).unwrap_or_default();
    let short: String = ref_id.chars().take(12).collect();
    let head = super::pages::f62d(
        "book-confirmed",
        "Request Received — CochranBlock",
        "Your discovery-call request has been received.",
    );
    let content = format!(
        r#"<section class="booking-page"><h1>Request received.</h1><p class="booking-intro">Reference: <code>{short}…</code></p><p>I'll confirm within 24 hours by reply to the email you gave. No payment, no prep — just the call.</p><p class="booking-fallback" style="margin-top:2rem">Need to reach me sooner? <a href="mailto:mcochran@cochranblock.org?subject=Discovery%20Call%20Follow-up">mcochran@cochranblock.org</a></p><p style="margin-top:2rem"><a href="/" class="btn btn-secondary">Back to CochranBlock</a></p></section>"#,
        short = short,
    );
    Html([head.as_str(), C7, &content, C8].concat())
}
