...
pub async fn f63(State(_p0): State<Arc<t0>>) -> Html<String> {
    ...
    let v0 = format!(
        r##"<section class="booking-page"><h1>Schedule a Discovery Call</h1><p class="booking-intro">30 minutes · Discuss your goals and how I can help · All times Eastern (EST)</p><p class="booking-context">Pick a date, then a time, then tell me a little about what you want to discuss.</p><p class="booking-legend">Available weekdays · 8am–5pm EST</p><p class="booking-hint" id="booking-hint">Select a date, then a time.</p><a href="#booking-calendar" class="skip-link skip-link-calendar">Skip to calendar</a><div class="booking-calendar-wrapper" id="booking-calendar" role="region" aria-label="Calendar - pick a date then a time" aria-describedby="booking-hint"><div class="booking-calendar-header"><div class="booking-month-row"><button type="button" class="booking-nav" id="booking-prev" aria-label="Previous month">&larr;</button><h3 class="booking-month" id="booking-month"></h3><button type="button" class="booking-nav" id="booking-next" aria-label="Next month">&rarr;</button><span class="booking-available-badge" id="booking-available-badge"></span></div></div><div class="booking-calendar-grid" id="booking-grid" role="grid" aria-label="Month view"></div></div><div class="booking-time-panel" id="booking-time-panel" aria-live="polite" hidden><h3 class="booking-time-heading" id="booking-time-heading"></h3><div class="booking-time-slots" id="booking-time-slots" role="group"></div></div><form id="booking-form" method="post" action="/book/submit" class="intake-form" hidden aria-hidden="true" style="margin-top:2rem"><p class="booking-form-intro">Requesting: <strong id="booking-form-when"></strong></p><input type="hidden" name="date" id="booking-form-date" value=""><input type="hidden" name="time" id="booking-form-time" value=""><label for="bk-name">Your name</label><input type="text" id="bk-name" name="name" required maxlength="200" autocomplete="name"><label for="bk-email">Your email</label><input type="email" id="bk-email" name="email" required maxlength="254" autocomplete="email"><label for="bk-phone">Phone <span class="intake-optional">(optional)</span></label><input type="tel" id="bk-phone" name="phone" maxlength="40" autocomplete="tel"><label for="bk-topic">What do you want to discuss?</label><textarea id="bk-topic" name="topic" rows="4" required maxlength="2000" placeholder="One or two sentences. What brings you here?"></textarea><div style="display:none" aria-hidden="true"><label>Leave blank<input type="text" name="website" tabindex="-1" autocomplete="off"></label></div><button type="submit" class="btn">Send Request</button><p class="booking-note" style="margin-top:0.75rem">I'll confirm within 24 hours by reply to the address above.</p></form><p class="booking-fallback">None of these work? <a href="mailto:mcochran@cochranblock.org?subject=Discovery%20Call%20Request">Email me directly</a> to propose a time.</p><script type="application/json" id="booking-slots-data">{}</script><script src="/assets/js/booking.js"></script></section>"##,
        slots_json_escaped
    );
    Html(format!(
        "{}{}{}{}",
