...
# Full deploy pipeline: build on bt (this node), hot-reload to gd, purge cache, ping search engines.

set -euo pipefail

# bt's interactive PATH points at Mac mounts — force a sane PATH so cargo, scp, ssh,
# curl all resolve to the local Linux binaries regardless of how this is invoked.
export PATH="$HOME/.cargo/bin:/usr/local/bin:/usr/bin:/bin:$PATH"

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$REPO_ROOT"

echo "[0/5] cargo audit gate (per .cargo/audit.toml)..."
if cargo audit > /tmp/cb-audit.log 2>&1; then
  echo "  PASS: 0 unignored vulnerabilities"
else
  echo "  ABORT: cargo audit found unignored advisories. Bump deps before deploy."
  tail -50 /tmp/cb-audit.log
  echo "  See: cd $REPO_ROOT && cargo audit"
  exit 1
fi

echo "[1/5] Build on bt..."
cargo build --release -p cochranblock --features approuter 2>&1 | tail -2

BIN="$REPO_ROOT/target/release/cochranblock"
[ -x "$BIN" ] || { echo "  ABORT: $BIN missing after build"; exit 1; }

echo "[2/5] Copy binary bt → gd (staging)..."
scp "$BIN" gd:/tmp/cochranblock-new
